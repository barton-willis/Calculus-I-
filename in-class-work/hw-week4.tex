...
\begin{solution}
Your TL might look different--here is mine

\begin{center}
\includegraphics[scale=0.25]{desmos-graph(32).png}
\end{center}
Again, your estimated dots on the TL might be different, but 
using the two labeled dots on the TL, the slope of the TL is
\[
  \frac{2.9 + 1.3}{4-0} \approx 1.0.
\]
...
\begin{equation}
  ...
  \begin{parts}
   ...
   \begin{center}
\includegraphics[scale=0.25]{desmos-graph(33).png}
\end{center}
\end{solution}
\newpage
   ...
   For $0<Y< 20$, the graph is a line with slope $0.035$. So for the interval \mbox{$0<Y< 20$}, we have  \(\displaystyle \frac{\mathrm{d} P}{\mathrm{d} Y} = 0.035\). For $20< Y$, the graph is a horizontal line with slope $0$. So for the interval $0<Y< 20$, we have  \(\displaystyle \frac{\mathrm{d} P}{\mathrm{d} Y} = 0\). At $Y = 20$, the graph doesn't have a TL, so the derivative at $20$ doesn't exist.  A graph of the derivative is
   
      \begin{center}
\includegraphics[scale=0.25]{desmos-graph(34).png}
\end{center}
\end{solution}
  \end{parts}
  ...
  \begin{align*}
    \frac{\mathrm{d}}{\mathrm{d} x} \left[ \cos(2) x^2 + \sin(2) x  + 46 \right] &= 
     \cos(2) \frac{\mathrm{d}}{\mathrm{d} x} \left[ x^2 \right ] + \sin(2)  \frac{\mathrm{d}}{\mathrm{d} x} \left[x\right]  +     \frac{\mathrm{d}}{\mathrm{d} x} \left [ 46 \right]  & \mbox{(Rule 1)} \\
     &=    \cos(2) (2 x ) + \sin(2)    +     0   & \mbox{(Rule 2)}  \\
     &= 2   \cos(2) x  + \sin(2)      & \mbox{(algebra)}  \\
  \end{align*}
Since $\cos(2)$ and $\sin(2)$ are both real numbers, we can use what I call the
outative property on them. There is a \textbf{huge} difference between
$\displaystyle  \frac{\mathrm{d}}{\mathrm{d} x} \left[ \cos(x) \right]$
and $\displaystyle  \frac{\mathrm{d}}{\mathrm{d} x} \left[ \cos(2) \right]$. So
be careful with that!
\end{solution}
   \part [1] \( \displaystyle \frac{\mathrm{d}}{\mathrm{d} x} \left[ \sin(3) x^2 + \ln(107) x  \right] \)
   \begin{solution}[2.0in]
    \begin{align*}
    \frac{\mathrm{d}}{\mathrm{d} x} \left[ \sin(3) x^2 + \ln(107) x  \right] &= 
     \sin(3) \frac{\mathrm{d}}{\mathrm{d} x} \left[ x^2 \right ] + \ln(107)   \frac{\mathrm{d}}{\mathrm{d} x} \left[x\right]  & \mbox{(Rule 1)} \\
     &=    \sin(3) (2 x ) + \ln(107)       & \mbox{(Rule 2)} \\
     &=    2 \sin(3) x + \ln(107)       & \mbox{(algebra)} \\
     \end{align*}
\end{solution}
    \part [1] \( \displaystyle \frac{\mathrm{d}}{\mathrm{d} x} \left[ \frac{x^2 + 1}{x}  \right] \)
   \begin{solution}%[2.0in]
    Yikes! We need the derivative of a quotient, but none of our rules match a quotient.
    What to do? We need to use some algebra to change the expression into something
    that matches one of our rules:
\begin{align*}
 \frac{\mathrm{d}}{\mathrm{d} x} \left[ \frac{x^2 + 1}{x}  \right] &=
  \frac{\mathrm{d}}{\mathrm{d} x} \left[ x +x^{-1} \right]  &\mbox{(algebra)} \\
  &= 1 - x^{-2} & \mbox{(Rule 2)}   
\end{align*}
\end{solution}
%\newpage
